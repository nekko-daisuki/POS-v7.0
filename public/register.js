document.addEventListener('DOMContentLoaded', function () {
    ...
    const closeCompletionScreenBtn = document.getElementById('closeCompletionScreenBtn');
    ...
    // 注文保存（★FormData送信に変更：プリフライト回避）
    async function saveToSpreadsheet(snapshot) {
        const form = new FormData();
        form.append('action', 'saveOrder');
        ...
        const resp = await fetch(GAS_WEB_APP_URL, { method: 'POST', body: form });
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
